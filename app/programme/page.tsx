...
export default function ProgrammePage() {
  ...
  return <main className="timeline-page"><section className="timeline-panel">
    ...
    {pending && <section style={{ padding: 20, marginBottom: 24, border: "1px solid #d7dde3", borderRadius: 18 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 12 }}>{(Object.keys(hierarchyLabels) as HierarchyField[]).map((field) => <label className="attendance-field" key={field}><span>{hierarchyLabels[field]}</span><select value={pending.mapping[field]} onChange={(event) => updateMapping(field, event.target.value)}><option value="">Leave unmapped</option><option value="__wbs__">WBS path fallback</option>{pending.parsed.availableColumns.map((column) => <option value={column} key={column}>{pending.parsed.columnLabels[column] || column}</option>)}</select></label>)}</div>
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 12 }}>{(["building", "elevation", "level", "gridline"] as const).map((field) => <span key={field} style={{ padding: "7px 10px", borderRadius: 999, background: "#eef2f4" }}><strong>{pending.parsed.activities.filter((activity) => Boolean(activity[field])).length}</strong> {hierarchyLabels[field]} values mapped</span>)}</div>
