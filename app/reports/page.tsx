...
export default function ReportsPage() {
  ...
  const report = useMemo(() => {
    ...
    const gangs = groupGangDayProductivity(completedWork.map(({ date, event }) => ({ date, event }))).map((group) => {
      const activity = programmeById.get(group.activityId);
      const source = completedWork.find(({ date, event }) => date === group.date && event.programmeActivityId === group.activityId && (event.crewId ?? "unassigned") === group.gangId);
      const gang = crewName(source?.day.crews ?? [], group.gangId === "unassigned" ? undefined : group.gangId);
      const related = weeklyEvents.filter(({ date, event }) => date === group.date && (event.crewId ?? "unassigned") === group.gangId);
      ...
      return { ...group, gang, activity: activity?.activity ?? group.activityId, unit: activity?.unit ?? "", planned, disruption, variation, performance: planned && group.actualManDayProductivity !== null ? group.actualManDayProductivity / planned * 100 : null };
    });
    ...
    return { weekEnd, dates, dayByDate, programmeById, operativeById, weeklyEvents, measured, activityPerformance, gangs, disruptions, variations, disruptionCost, disruptionLostHours, weeklyAttendanceHours, weeklyBackshiftHours, weeklyAttendanceCost, labourByDate, narrative, productivityStart, productivityEnd };
  ...
  return <main className="timeline-page"><section className="timeline-panel">
    ...
    <Section number={2} title="Measured Work / Productivity">{report.measured.length === 0 ? <p>No completed measured work recorded in this period.</p> : <Table minWidth={2200}><thead><tr>{["Building", "Elevation", "Level", "Activity", "Activity ID", "Unit", "Quantity Completed", "Daily Gang Output", "Cumulative Quantity", "Planned Quantity", "Remaining", "% Complete", "Planned Gang Size", "Actual Operative Man-Days", "Planned Man-Day Productivity", "Actual Man-Day Productivity", "Productivity Performance %", "Disruption Labour Hours", "Productive Labour Hours", "Man-Hour Productivity (Advanced)"].map((heading) => <th key={heading} style={cellStyle}>{heading}</th>)}</tr></thead><tbody>{report.measured.map((row) => <tr key={row.programmeActivityId}><td style={cellStyle}>{row.building || "—"}</td><td style={cellStyle}>{row.elevation || "—"}</td><td style={cellStyle}>{row.level || "—"}</td><td style={cellStyle}>{row.activity}</td><td style={cellStyle}>{row.programmeActivityId}</td><td style={cellStyle}>{row.unit || "—"}</td><td style={cellStyle}>{number(row.weeklyQuantity)}</td><td style={cellStyle}>{row.dailyGangOutput === null ? "—" : number(row.dailyGangOutput)}</td><td style={cellStyle}>{number(row.cumulativeQuantity)}</td><td style={cellStyle}>{number(row.plannedQuantity)}</td><td style={cellStyle}>{number(row.remaining)}</td><td style={cellStyle}>{number(row.percentage)}%</td>{row.baselineComplete ? <><td style={cellStyle}>{number(row.assumedGangSize ?? 0)}</td><td style={cellStyle}>{number(row.manDays)}</td><td style={cellStyle}>{number(row.plannedRate ?? 0)}</td><td style={cellStyle}>{row.actualRate === null ? "—" : number(row.actualRate)}</td><td style={cellStyle}>{row.productivityPerformance === null ? "—" : `${number(row.productivityPerformance)}%`}</td><td style={cellStyle}>{number(row.disruptionHours)}</td><td style={cellStyle}>{number(row.hours)}</td><td style={cellStyle}>{row.manHourRate === null ? "—" : number(row.manHourRate)}</td></> : <td colSpan={9} style={{ ...cellStyle, color: "#b42318", fontWeight: 700 }}>Man-day productivity baseline required</td>}</tr>)}</tbody></Table>}</Section>
