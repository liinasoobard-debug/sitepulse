...
import { groupGangDayProductivity } from "@/lib/manDayProductivity";
import ProductivityRagBadge from "@/components/ProductivityRagBadge";
import { productivityRag, ragDistribution, type ProductivityRag } from "@/lib/productivityRag";
...
export default function ReportsPage() {
  ...
  const report = useMemo(() => {
    ...
    const measured = programme.flatMap((activity) => {
      ...
      return [{
        ...
        productivityRag: productivityRag(plannedRate, actualRate),
    ...
    const gangs = groupGangDayProductivity(productivityCompletedWork.map(({ date, event }) => ({ date, event }))).map((group) => {
      ...
      return { ...group, gang, activity: activity?.activity ?? group.activityId, unit: activity?.unit ?? "", planned, disruption, variation, performance: planned && group.actualManDayProductivity !== null ? group.actualManDayProductivity / planned * 100 : null, productivityRag: productivityRag(planned, group.actualManDayProductivity) };
    });
    ...
    const topRed = measured.filter((row) => row.productivityRag === "red").sort((a, b) => (a.productivityPerformance ?? 0) - (b.productivityPerformance ?? 0));
  ...
  function exportExcel() {
    ...
    const measuredRows = report.measured.map((row) => ({ Activity: row.activity, "Activity ID": row.programmeActivityId, Building: row.building, Elevation: row.elevation, Level: row.level, Unit: row.unit, "Productivity RAG": row.productivityRag, "Quantity Completed": row.weeklyQuantity, "Daily Gang Output": row.dailyGangOutput ?? "", "Cumulative Quantity": row.cumulativeQuantity, "Planned Quantity": row.plannedQuantity, "Planned Gang Size": row.assumedGangSize ?? "", "Actual Operative Man-Days": row.manDays, "Planned Man-Day Productivity": row.plannedRate ?? "", "Actual Man-Day Productivity": row.actualRate ?? "", "Productivity Performance %": row.productivityPerformance ?? "", "Main Blocker": row.mainBlocker, "Disruption Labour Hours": row.disruptionHours, "Productive Labour Hours": row.hours, "Man-Hour Productivity (Advanced)": row.manHourRate ?? "" }));
  ...
  return <main className="timeline-page"><section className="timeline-panel">
    ...
    <section aria-label="Productivity RAG report summary"><h2>Productivity RAG Summary</h2><p>Productivity RAG is independent from Programme RAG.</p><div className="productivity-rag-summary"><div><strong>{report.ragSummary.counts.green}</strong><span>Green · {number(report.ragSummary.percentages.green)}%</span></div><div><strong>{report.ragSummary.counts.amber}</strong><span>Amber · {number(report.ragSummary.percentages.amber)}%</span></div><div><strong>{report.ragSummary.counts.red}</strong><span>Red · {number(report.ragSummary.percentages.red)}%</span></div><div><strong>{report.ragSummary.counts["baseline-missing"]}</strong><span>Baseline Missing</span></div><div><strong>{report.ragSummary.counts["no-actuals"]}</strong><span>No Actuals</span></div></div><h3>Top Red activities and blockers</h3>{report.topRed.length ? <Table><thead><tr><th style={cellStyle}>RAG</th><th style={cellStyle}>Activity</th><th style={cellStyle}>Performance</th><th style={cellStyle}>Main blocker</th></tr></thead><tbody>{report.topRed.slice(0, 10).map((row) => <tr key={row.programmeActivityId}><td style={cellStyle}><ProductivityRagBadge status="red" /></td><td style={cellStyle}>{row.activity}</td><td style={cellStyle}>{number(row.productivityPerformance ?? 0)}%</td><td style={cellStyle}>{row.mainBlocker}</td></tr>)}</tbody></Table> : <p>No Red activities in this period.</p>}<h3>RAG movement over time</h3>{report.movement.length ? <ul>{report.movement.map((row) => <li key={row.activity}>{row.activity}: <ProductivityRagBadge status={row.from as ProductivityRag} /> → <ProductivityRagBadge status={row.to as ProductivityRag} /></li>)}</ul> : <p>No Red/Amber/Green movement recorded within this period.</p>}</section>
    ...
    <Section number={2} title="Measured Work / Productivity">{report.measured.length === 0 ? <p>No completed measured work recorded in this period.</p> : <Table minWidth={2350}><thead><tr>{["Building", "Elevation", "Level", "Activity", "Activity ID", "Unit", "Productivity RAG", "Quantity Completed", "Daily Gang Output", "Cumulative Quantity", "Planned Quantity", "Remaining", "% Complete", "Planned Gang Size", "Actual Operative Man-Days", "Planned Man-Day Productivity", "Actual Man-Day Productivity", "Productivity Performance %", "Main Blocker", "Disruption Labour Hours", "Productive Labour Hours", "Man-Hour Productivity (Advanced)"].map((heading) => <th key={heading} style={cellStyle}>{heading}</th>)}</tr></thead><tbody>{report.measured.map((row) => <tr key={row.programmeActivityId}><td style={cellStyle}>{row.building || "—"}</td><td style={cellStyle}>{row.elevation || "—"}</td><td style={cellStyle}>{row.level || "—"}</td><td style={cellStyle}>{row.activity}</td><td style={cellStyle}>{row.programmeActivityId}</td><td style={cellStyle}>{row.unit || "—"}</td><td style={cellStyle}><ProductivityRagBadge status={row.productivityRag} /></td><td style={cellStyle}>{number(row.weeklyQuantity)}</td><td style={cellStyle}>{row.dailyGangOutput === null ? "—" : number(row.dailyGangOutput)}</td><td style={cellStyle}>{number(row.cumulativeQuantity)}</td><td style={cellStyle}>{number(row.plannedQuantity)}</td><td style={cellStyle}>{number(row.remaining)}</td><td style={cellStyle}>{number(row.percentage)}%</td><td style={cellStyle}>{row.assumedGangSize ? number(row.assumedGangSize) : "—"}</td><td style={cellStyle}>{number(row.manDays)}</td><td style={cellStyle}>{row.plannedRate === null ? "—" : number(row.plannedRate)}</td><td style={cellStyle}>{row.actualRate === null ? "—" : number(row.actualRate)}</td><td style={cellStyle}>{row.productivityPerformance === null ? "—" : `${number(row.productivityPerformance)}%`}</td><td style={cellStyle}>{row.mainBlocker}</td><td style={cellStyle}>{number(row.disruptionHours)}</td><td style={cellStyle}>{number(row.hours)}</td><td style={cellStyle}>{row.manHourRate === null ? "—" : number(row.manHourRate)}</td></tr>)}</tbody></Table>}</Section>
    ...
    <Section number={3} title="Productivity by Gang">{report.gangs.length === 0 ? <p>No gang productivity records available.</p> : <Table minWidth={1450}><thead><tr>{["Date", "Gang", "Activity", "Unit", "Daily Gang Output", "Gang Size", "Planned Man-Day Productivity", "Actual Man-Day Productivity", "Performance %", "Productive Labour Hours", "Disruption Labour Hours", "VO / Change Labour Hours"].map((heading) => <th key={heading} style={cellStyle}>{heading}</th>)}</tr></thead><tbody>{report.gangs.map((row) => <tr key={row.key}><td style={cellStyle}>{formatDay(row.date)}</td><td style={cellStyle}>{row.gang}</td><td style={cellStyle}>{row.activity}</td><td style={cellStyle}>{row.unit || "—"}</td><td style={cellStyle}>{number(row.quantity)}</td><td style={cellStyle}>{row.operatives || "—"}</td><td style={cellStyle}>{row.planned === null ? "—" : number(row.planned)}</td><td style={cellStyle}>{row.actualManDayProductivity === null ? "—" : number(row.actualManDayProductivity)}</td><td style={cellStyle}>{row.performance === null ? "—" : `${number(row.performance)}%`}</td><td style={cellStyle}>{number(row.productiveHours)}</td><td style={cellStyle}>{number(row.disruption)}</td><td style={cellStyle}>{number(row.variation)}</td></tr>)}</tbody></Table>}</Section>
