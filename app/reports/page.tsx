...
function Section({ number: sectionNumber, title, children }: { number: number; title: string; children: React.ReactNode }) {
  return <section style={{ marginBottom: 34 }}><p className="eyebrow">Section {sectionNumber}</p><h2>{title}</h2>{children}</section>;
}
...
export default function ReportsPage() {
  ...
  const report = useMemo(() => {
    ...
    const narrative = days.length === 0 ? "Enter or backdate daily records for this week to populate the report." : [
      `During the week commencing ${formatDate(weekStart)}, the site recorded ${number(weeklyAttendanceHours)} labour hours across ${gangNames.size} gang${gangNames.size === 1 ? "" : "s"}.`,
  ...
  return <main className="timeline-page"><section className="timeline-panel">
    <header className="timeline-header"><div><p className="eyebrow">Production reporting</p><h1>Weekly Production Report</h1><p style={{ marginBottom: 0, color: "#5f6b76" }}>{project?.name ?? "Project"}</p></div><span className="status-pill">Read only</span></header>
