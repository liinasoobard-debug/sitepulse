...
export default function ReportsPage() {
  ...
  const report = useMemo(() => {
    ...
    const measured = programme.flatMap((activity) => {
      ...
      const plannedCrewOutput = plannedRate && activity.plannedCrewSize ? plannedRate * activity.plannedCrewSize : null;
      return [{
        ...activity,
        weeklyQuantity,
        cumulativeQuantity,
        ...
        baselineComplete: Boolean(activity.plannedQuantity > 0 && activity.unit && plannedRate && (activity.plannedCrewSize ?? 0) > 0),
        plannedRate,
        plannedCrewOutput,
        actualRate,
        overallRate,
  ...
  return <main className="timeline-page"><section className="timeline-panel">
    ...
    <Section number={2} title="Measured Work Achievement">{report.measured.length === 0 ? <p>No completed measured work recorded in this period.</p> : <Table minWidth={2800}><thead><tr>{["Building", "Elevation", "Level", "Activity", "Activity ID", "Unit", "Quantity in Period", "Cumulative Quantity", "Planned Quantity", "Remaining", "% Complete", "Baseline Men", "Planned Crew Output / Hour", "Planned Production Rate", "Actual Production Rate", "Overall Production Rate", "Productive Labour Hours", "Disruption Labour Hours", "Earned Labour Hours", "Labour Productivity Index", "Overall Labour Efficiency Index", "Productivity Performance %"].map((heading) => <th key={heading} style={cellStyle}>{heading}</th>)}</tr></thead><tbody>{report.measured.map((row) => <tr key={row.programmeActivityId}><td style={cellStyle}>{row.building || "—"}</td><td style={cellStyle}>{row.elevation || "—"}</td><td style={cellStyle}>{row.level || "—"}</td><td style={cellStyle}>{row.activity}</td><td style={cellStyle}>{row.programmeActivityId}</td><td style={cellStyle}>{row.unit || "—"}</td><td style={cellStyle}>{number(row.weeklyQuantity)}</td><td style={cellStyle}>{number(row.cumulativeQuantity)}</td><td style={cellStyle}>{number(row.plannedQuantity)}</td><td style={cellStyle}>{number(row.remaining)}</td><td style={cellStyle}>{number(row.percentage)}%</td>{row.baselineComplete ? <><td style={cellStyle}>{number(row.plannedCrewSize ?? 0)}</td><td style={cellStyle}>{number(row.plannedCrewOutput ?? 0)}</td><td style={cellStyle}>{number(row.plannedRate ?? 0)}</td><td style={cellStyle}>{row.actualRate === null ? "—" : number(row.actualRate)}</td><td style={cellStyle}>{row.overallRate === null ? "—" : number(row.overallRate)}</td><td style={cellStyle}>{number(row.hours)}</td><td style={cellStyle}>{number(row.disruptionHours)}</td><td style={cellStyle}>{row.earnedHours === null ? "—" : number(row.earnedHours)}</td><td style={cellStyle}>{row.labourProductivityIndex === null ? "—" : number(row.labourProductivityIndex)}</td><td style={cellStyle}>{row.overallRate === null ? "—" : number(row.overallLabourEfficiencyIndex ?? 0)}</td><td style={cellStyle}>{row.productivityPerformance === null ? "—" : `${number(row.productivityPerformance)}%`}</td></> : <td colSpan={11} style={{ ...cellStyle, color: "#b42318", fontWeight: 700 }}>Productivity baseline incomplete</td>}</tr>)}</tbody></Table>}</Section>
