...
export async function POST(request: Request) {
  ...
  const form=await request.formData(); const file=form.get("file"); const projectId=String(form.get("projectId")??""); const building=String(form.get("building")??"").trim(); const hoursPerManDay=Number(form.get("hoursPerManDay")); let sourceType=String(form.get("sourceType")??"p6-xlsx") as ProgrammeImportSource;
  ...
  try {
    ...
    if(sourceType==="p6-xlsx") { const first=parseP6Workbook(sheets,projectId,importId,empty,knownIds,hoursPerManDay); const candidates=first.availableColumns.map(column=>({column,key:`${column} ${first.columnLabels[column]??""}`.toLowerCase().replace(/[\s_-]+/g," ")})); mapping={...empty}; (Object.keys(mapping) as HierarchyField[]).forEach(field=>{if(mapping[field])return;const label=hierarchyLabels[field].toLowerCase();mapping[field]=candidates.find(c=>c.key.includes(label)||field==="level"&&c.key.includes("floor")||field==="workActivity"&&c.key.includes("activity name"))?.column??"";}); parsed=parseP6Workbook(sheets,projectId,importId,mapping,knownIds,hoursPerManDay); }
    ...
    const activityRows=parsed.activities.map(a=>({id:a.id,project_id:projectId,programme_import_id:importId,external_activity_id:a.programmeActivityId,activity_name:a.activityName||a.activity,activity_status:a.activityStatus||null,wbs_code:a.wbsCode||null,wbs_name:a.wbsPath||null,building:a.building||null,area:a.elevation||null,level:a.level||null,gridline:a.gridline||null,location:[a.building,a.elevation,a.level,a.gridline].filter(Boolean).join(" / ")||null,trade:a.trade||null,planned_start:a.plannedStart||null,planned_finish:a.plannedFinish||null,actual_start:a.actualStart||null,actual_finish:a.actualFinish||null,original_duration:a.originalDuration??null,remaining_duration:a.remainingDuration??null,percent_complete:a.physicalPercentComplete??null,planned_quantity:a.plannedQuantity||null,unit:a.unit||null,productivity_target:a.plannedProductionRate??null,planned_crew_size:a.plannedCrewSize??null,planned_man_day_productivity:a.plannedManDayProductivity??null,assumed_gang_size:a.assumedGangSize??null,planned_gang_daily_output:a.plannedGangDailyOutput??null,planned_man_days:a.plannedManDays??null,planned_duration_days:a.plannedDurationDays??null,calendar_name:a.calendar||null,is_missing_from_latest:false,created_at:importedAt,updated_at:importedAt,raw_data:{productType:a.productType||null,programmeStatus:a.status||null,budgetLabourHours:a.budgetLabourHours??null,sourceType,sourceFilename:file.name,importDate:importedAt,importedBy:user.id}}));
