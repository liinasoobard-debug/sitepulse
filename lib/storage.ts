import type { Operative, Project, SiteDay } from "@/types/site";
...
export const ACTIVE_PROJECT_STORAGE_KEY = "sitepulse-active-project";
...
function migrateLegacyDay(projectId: string): void {
  ...
  const legacyDay = localStorage.getItem(LEGACY_DAY_STORAGE_KEY);
  if (!legacyDay) return;
  ...
  try {
    ...
    console.error("Unable to migrate the existing site day:", error);
  }
}

function ensureProjectSetup(): { projects: Project[]; activeProjectId: string } {
    ...
    if (storedProjects) {
      ...
      if (Array.isArray(parsedProjects)) {
        projects = parsedProjects
          .map((project) => normaliseProject(project as Project))
  ...
  const storedActiveProjectId = localStorage.getItem(ACTIVE_PROJECT_STORAGE_KEY);
  const activeProjectExists = projects.some(
    (project) => project.id === storedActiveProjectId && !project.isArchived
  );

  const activeProjectId = activeProjectExists
    ? String(storedActiveProjectId)
    : projects.find((project) => !project.isArchived)?.id ?? projects[0].id;
...
export function setActiveProject(projectId: string): void {
  ...
  window.dispatchEvent(
    new CustomEvent("sitepulse-project-changed", { detail: { projectId } })
  );
}
...
export function updateProject(updatedProject: Project): Project[] {
  const updatedProjects = loadProjects().map((project) =>
    project.id === updatedProject.id ? normaliseProject(updatedProject) : project
  );
  saveProjects(updatedProjects);
  return updatedProjects;
}
...
export function loadDay(projectId?: string): SiteDay | null {
  ...
  try {
    ...
    const data = localStorage.getItem(getProjectDayStorageKey(resolvedProjectId));
...
export function saveDay(data: SiteDay, projectId?: string): void {
  if (typeof window === "undefined") return;
...
export function loadOperatives(): Operative[] {
  ...
  try {
    const savedData = localStorage.getItem(OPERATIVES_STORAGE_KEY);
    if (savedData) {
      const parsedData = JSON.parse(savedData);
      if (Array.isArray(parsedData)) return parsedData as Operative[];
    }
...
export function saveOperatives(operatives: Operative[]): void {
  if (typeof window === "undefined") return;
...
export function addOperative(operative: Operative): Operative[] {
  const existingOperatives = loadOperatives();
  if (existingOperatives.some((item) => item.id === operative.id)) {
    return existingOperatives;
  }

  const updatedOperatives = [...existingOperatives, operative];
  saveOperatives(updatedOperatives);
  return updatedOperatives;
}

export function updateOperative(updatedOperative: Operative): Operative[] {
  const updatedOperatives = loadOperatives().map((operative) =>
    operative.id === updatedOperative.id ? updatedOperative : operative
  );
  saveOperatives(updatedOperatives);
  return updatedOperatives;
}

export function deleteOperative(operativeId: string): Operative[] {
