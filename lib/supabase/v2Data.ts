...
export async function loadV2DailyRecords(projectId: string): Promise<V2DailyRecord[]> {
  const { data, error } = await createClient().from("timeline_events").select("id,event_date,external_activity_id,actual_quantity,labour_hours,change_category,note").eq("project_id", projectId).eq("event_type", "work").eq("status", "completed").is("deleted_at", null).order("event_date");
