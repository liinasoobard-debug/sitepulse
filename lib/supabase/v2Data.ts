...
export async function loadV2DailyRecords(projectId: string): Promise<V2DailyRecord[]> {
  ...
  const { data, error } = await supabase.from("timeline_events").select("id,event_date,external_activity_id,actual_quantity,labour_hours,operative_count,lost_labour_hours,change_category,note").eq("project_id", projectId).eq("event_type", "work").eq("status", "completed").is("deleted_at", null).order("event_date");
  if (error) throw error;
  return (data ?? []).flatMap((row) => row.external_activity_id ? [{ id: String(row.id), date: String(row.event_date), activityId: String(row.external_activity_id), quantity: Number(row.actual_quantity ?? 0), labourHours: Number(row.labour_hours ?? 0), men: row.operative_count === null ? undefined : Number(row.operative_count), affectedHours: row.lost_labour_hours === null ? undefined : Number(row.lost_labour_hours), constrained: Boolean(row.change_category), constraintReason: row.change_category ? String(row.change_category) : undefined, note: row.note ? String(row.note) : undefined }] : []);
}
